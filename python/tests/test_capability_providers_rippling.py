...
def _rippling_rule(action, resource, context):
    ...
    return {"allowed": allowed, "reason": None if allowed else "bonus policy: over cap without exception / unauthorized"}


@pytest.mark.parametrize("cid, action, authorized, amount, has_exc, expected", SCENARIOS)
def test_user_schema_provider_matches_rippling_policy(cid, action, authorized, amount, has_exc, expected):
